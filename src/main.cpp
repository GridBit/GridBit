...
unsigned int nStakeMinAge = 60 * 60 * 2 * 1;
unsigned int nStakeMaxAge = 60 * 60 * 24 * 90;

static const int MODIFIER_INTERVAL_SWITCH = 4900; // pos start block

int64 nChainStartTime = 1408021256;
...
const string strMessageMagic = "GridBit Signed Message:\n";
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in GridBit
    bool fStrictPayToScriptHash = true; // Always active in GridBit
...
bool CBlock::CheckBlock(bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Check proof of work matches claimed amount
    if (fCheckPOW && IsProofOfWork() && !ChekProofOfWork(GetHash(), nBits))
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "GridBit", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 47103 << CBigNum(587) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        ...
        assert(block.hashMerkleRoot == uint256("0x6f81666725eca8da447d1cfaac5407d08e20720c318e0f332c29eb3b95ec522a"));
...
unsigned char pchMessageStart[4] = { 0xf7, 0xfa, 0xb4, 0xb4 };
